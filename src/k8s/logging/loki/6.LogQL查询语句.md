...
![](https://im.xinn.cc/xxlog/1722392115082-f7b285e4-77f2-4e23-9d69-6883aa6a0430.png)
...
![](https://im.xinn.cc/xxlog/1722392113380-92839ad4-0e40-451e-89ad-3162899bf690.jpeg)
...
![](https://im.xinn.cc/xxlog/1722392114443-83ddfbd6-30d1-4d8e-8e14-a38808d7120e.jpeg)

由于我们该示例应用的日志是 JSON 形式的，我们可以采用 JSON 解析器来解析日志，表达式为 `{app="fake-logger"} | json`，如下所示。

![](https://im.xinn.cc/xxlog/1722392114700-c81c9e10-9d4a-4c30-9b69-a0846d315c19.jpeg)

使用 JSON 解析器解析日志后可以看到 Grafana 提供的面板会根据 `level` 的值使用不同的颜色进行区分，而且现在我们日志的属性也被添加到了 Log 的标签中去了。

![](https://im.xinn.cc/xxlog/1722392113705-c19625d8-aed4-4ea6-835f-c9031d31b797.png)

现在 JSON 中的数据变成了日志标签我们自然就可以使用这些标签来过滤日志数据了，比如我们要过滤 `level=error` 的日志，只使用表达式 `{app="fake-logger"} | json | level="error"` 即可实现。

![](https://im.xinn.cc/xxlog/1722392114682-d9cea591-8035-4765-84fd-f3b9d4c96b3f.jpeg)
...
![](https://im.xinn.cc/xxlog/1722392114882-bdd06dce-8c92-4213-98f0-6ab1b36e180c.jpeg)
...
![](https://im.xinn.cc/xxlog/1722392115330-487a6d8c-9a28-4b2c-badc-07567089ebc7.jpeg)
...
![](https://im.xinn.cc/xxlog/1722392115500-150177fd-8742-4716-a3ef-fe488f536564.png)

修改后正常就可以在 Dashboard 中看到集群中的相关事件信息了。

![](https://im.xinn.cc/xxlog/1722392116131-5c59eba5-5cac-4e8b-9edd-74959c9058af.jpeg)
...
![](https://im.xinn.cc/xxlog/1722392117064-f232c1f6-33bc-471b-95eb-331ad80c8ad3.png)
