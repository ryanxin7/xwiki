...
### kube-apiserver
**API Server 提供了资源对象的唯一操作入口**，其它所有组件都必须通过它提供的 API 来操作资源数据。**只有 API Server 会与 etcd 进行通信，其它模块都必须通过 API Server 访问集群状态**。<br />API Server 作为 Kubernetes 系统的入口，封装了核心对象的增删改查操作。<br />**API Server 以 RESTFul 接口方式提供给外部客户端和内部组件调用**，API Server 再对相关的资源数据（全量查询 + 变化监听）进行操作，以达到实时完成相关的业务功能。<br />以 API Server 为 Kubernetes 入口的设计主要有以下好处：
